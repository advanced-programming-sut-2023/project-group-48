...
public enum SignUpMenuCommands {
    CREATEUSER {"user\\s+create" , "\\s+-u\\s+(?<username>\\S+)" , "\\s+-p\\s+(?<password>\\S+)\\s+(?<passconfirm>\\S+)", "\\s+-email\\s+(?<email>\\S+)" , "\\s+-n\\s+(?<nickname>\\S+)" , "(\\s+-s\\s+(?<slogan>\".*\"))?" } ,
    ANSWERSECURITYWQUESTION {"question\\s+pick" , "\\s+-q\\s+(?<questionnumber>\\d)" , "\\s+-a\\s+(?<answer>\\S+)" , "\\s+-c\\s+(?<confirm>\\S+)" }
    ;
